...
/**
 * POST /live/guest-register
 * Creates or retrieves an anonymous guest user using a UUID from the frontend.
 * No Strava auth required. guestId is generated by the frontend (crypto.randomUUID)
 * and persisted in localStorage. This allows live tracking without any signup.
 */
app.post("/live/guest-register", async (req, res) => {
  try {
    ...
    if (!user) throw new Error("Could not create or find guest user");
